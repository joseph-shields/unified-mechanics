...
\begin{abstract}
The single recursion $\varphi^2 = \varphi + 1$ fixes the contraction rate
...
\textbf{Notation.} Residuals are normalised by the accumulated braiding
floor $n\,\varepsilon_{\text{floor}}$, where $n$ is the number of
independent recursion steps in the derivation --- equivalently, the
power of $r$ or $\varphi$ in the formula. A result below
$2\,n\,\varepsilon_{\text{floor}}$ is consistent with the framework.

For $\Delta H_0/H_0 = 3r^3$ the formula contains $r^3$ (three powers of
$r$), giving $n = 3$ and accumulated floor $3\,\varepsilon_{\text{floor}}
= 8.85\%$. The observed tension of $8.31\%$ gives a residual of $6.5\%$,
or $0.74\,n\,\varepsilon$.
...
\midrule
\multicolumn{5}{l}{\textit{Electroweak and baryon sector}} \\
...
$M_H/M_{\text{Pl}}$ & $r^{33}(1-r)$ & $1.021\!\times\!10^{-17}$ & $+0.26\%$ & $0.003$ \\
\midrule
\multicolumn{5}{l}{\textit{Lepton sector}} \\
$m_e/m_\mu$        & $\varphi^{-11}/(1+r^3)$ & $0.004881$ & $+0.93\%$ & $0.029$ \\
$m_\mu/m_\tau$     & $\varphi^{-6}(1+r^3)/(1-r^3)$ & $0.05912$ & $-0.58\%$ & $0.033$ \\
$m_e/m_\tau$       & $\varphi^{-17}/(1-r^3)$ & $0.000289$ & $+0.19\%$ & $0.004$ \\
$m_{\nu_3}$        & $m_e(1+r)/\varphi^{34}$ & $52.5\,\text{meV}$ & $+4.9\%$ & $0.049$ \\
$m_{\nu_2}$        & $m_e(1+r)/\varphi^{38}$ & $7.65\,\text{meV}$ & $-11.0\%$ & $0.098$ \\
\midrule
\multicolumn{5}{l}{\textit{Quark and CKM sector}} \\
$m_d/m_u$          & $\sqrt{W_B/W_M}$ & $2.115$ & $-2.2\%$ & $0.74$ \\
$m_u$              & $2\varphi^4 m_e/(1+\rho)$ & $2.249\,\text{MeV}$ & $+4.1\%$ & $0.35$ \\
$m_d$              & $2\varphi^4 m_e\rho/(1+\rho)$ & $4.756\,\text{MeV}$ & $+1.8\%$ & $0.16$ \\
$\lambda=|V_{us}|$ & $1/\varphi^3$ & $0.23607$ & $+5.2\%$ & $0.58$ \\
$|V_{cb}|$         & $W_B \times W_M$ & $0.04078$ & $-0.9\%$ & $0.14$ \\
$A$                & $W_B W_M\varphi^6$ & $0.7318$ & $-0.9\%$ & $0.14$ \\
$\delta_{\text{CP}}$ & $\arccos(W_B)$ & $1.130\,\text{rad}$ & $-0.9\%$ & $0.31$ \\
\midrule
\multicolumn{5}{l}{\textit{Fine structure constant}} \\
$1/\alpha_{\text{em}}$ & $\varphi^{10}+\varphi^5+\varphi^2+\varphi^{-2}$ & $137.082$ & $+0.034\%$ & $0.001$ \\
\midrule
\multicolumn{5}{l}{\textit{Cosmological sector}} \\
$n_s$              & $1-r^2/\varphi^2$ & $0.9635$ & $-0.14\%$ & $0.048$ \\
$\Delta H_0/H_0$   & $3r^3$ & $8.85\%$ & $+6.5\%$ & $0.74\;(n{=}3)$ \\
$r_s$ (BAO)        & EFTCAMB/UM & $148.1\,\text{Mpc}$ & $+0.61\%$ & --- \\
$\eta_B$           & $r^{18}$ & $6.60\!\times\!10^{-10}$ & $+8.2\%$ & $0.15$ \\
$\Lambda/M_{\text{Pl}}^4$ & $r^{241}$ & $1.22\!\times\!10^{-123}$ & $+5.5\%$ & $0.008$ \\
\bottomrule
\end{tabular}
\end{center}

where $\rho = \sqrt{W_B/W_M} = \sqrt{2\sqrt{5}}$ and $n\,\varepsilon$
denotes the residual in units of the accumulated braiding floor
$n\,\varepsilon_{\text{floor}}$ at cycle depth $n$.

\textbf{Twenty-one observables. Zero free parameters. Twenty-one within
$2\,n\,\varepsilon_{\text{floor}}$.} Every entry in the Standard Model
requires an independently measured parameter. UM derives each from
$\varphi^2 = \varphi + 1$.
...
What remains:
\begin{itemize}
  \item \textbf{Strange quark.} The boundary-channel estimate
        $\varphi^{12}m_e\sqrt{W_B} = 107.5\,\text{MeV}$ gives
        $0.43\,n\,\varepsilon$ ($n = 12$). The $\sqrt{W_B}$ prefactor
        requires derivation from the $G_2$ zero-mode structure.
  \item \textbf{$G_2$ exponent derivation.} The winding numbers
        $\{10, 5, 2, -2\}$ for $1/\alpha_{\text{em}}$ are fixed by the
        $G_2$ root system (Paper~6); the companion heterotic paper
        provides the explicit cycle-integration proof.
  \item \textbf{Wolfenstein $\bar\rho$, $\bar\eta$.} The remaining
        CKM parameters require $G_2$-sector derivation to make the
        Jarlskog invariant $J$ fully first-principles.
\end{itemize}
...
Twenty-one of twenty-one within the accumulated braiding floor. The Standard
Model requires at least nineteen independently measured parameters across
the same observables. UM requires one axiom and the mathematics that
follows from it.
