...
$+4.9\%$ ($1.66\,\varepsilon_{\text{floor}}$).  The solar scale
follows at depth $n = 38$:
$m_{\nu_2} = m_e(1{+}r)/\varphi^{38} = 7.65\;\text{meV}$,
within $3.73\,\varepsilon_{\text{floor}}$ of
$\sqrt{\Delta m^2_{\text{sol}}} \approx 8.6\;\text{meV}$.
...
\[
  \text{Residual} = +4.9\% = 1.66\,\varepsilon_{\text{floor}}.
\]

The agreement is within two braiding floors, the natural precision
limit of the framework for quantities where QCD and electroweak
radiative corrections have not yet been incorporated into the cycle
boundary.
...
\[
  \text{Residual} = -11.0\% = -3.73\,\varepsilon_{\text{floor}}.
\]

The $m_{\nu_2}$ result sits outside one braiding floor, at
$3.7\,\varepsilon_{\text{floor}}$.  This is consistent with
the interpretation that QCD running affects the exact cycle
boundary for the lighter neutrino states more strongly than for
$m_{\nu_3}$: the solar scale is two orders of magnitude smaller
than the atmospheric scale, and corrections from the strong sector
that are negligible at $m_{\nu_3}$ become more significant at
$m_{\nu_2}$.  Incorporating the running of the UM-derived
$\alpha_s(M_Z) = r^2(1+r-r^2)$ down to the neutrino mass scale
is identified as the next required calculation.
